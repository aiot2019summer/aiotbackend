...
def write_results(prediction, confidence, num_classes, nms = True, nms_conf = 0.4):
    ...
    if 0 in img_classes.float():
        #get the detections with one particular class
        cls_mask = image_pred_*(image_pred_[:,-1] == 0).float().unsqueeze(1)
        ...
        batch_ind = image_pred_class.new(image_pred_class.size(0), 1).fill_(0)
